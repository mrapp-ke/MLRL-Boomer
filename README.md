...
| `--print-options`                | Yes       | `None`                | Additional options to be used for printing or storing rules, if the parameter `--print-rules` or `--store-rules` is set to `True`. The options must be provided using the bracket notation `{print_feature_names=True,print_label_names=True,print_nominal_values=True}`.                                                                                                                                             |
...
| `--current-fold`                 | Yes       | `0`                   | The cross-validation fold to be performed or `0`, if all folds should be performed. Must be `0` or at least `1` and less or equal to `--folds`. If `--folds` is 1, this parameter is ignored.                                                                                                                                                                                                                         |
| `--max-rules`                    | Yes       | `1000`                | The number of rules to be induced or `0`, if the number of rules should not be restricted.                                                                                                                                                                                                                                                                                                                            |
| `--default-rule`                 | Yes       | `True`                | `True`, if the first rule should be a default rule, `False` otherwise.                                                                                                                                                                                                                                                                                                                                                |
| `--time-limit`                   | Yes       | `0`                   | The duration in seconds after which the induction of rules should be canceled or `0`, if no time limit should be used.                                                                                                                                                                                                                                                                                                |
| `--early-stopping`               | Yes       | `None`                | The name of the strategy to be used for early stopping. Must be `loss` or `None`, if no early stopping should be used. Additional options may be provided using the bracket notation `loss{min_rules=100,update_interval=1,stop_interval=1,num_past=50,num_recent=50,aggregation=min,tolerance=0.001}`. Does only have an effect if the parameter `--holdout` is not set to `None`.                                   |                                                                                
| `--label-sampling`               | Yes       | `None`                | The name of the strategy to be used for label sampling. Must be `without-replacement` or `None`. Additional options may be provided using the bracket notation `without-replacement{num_samples=5}`.                                                                                                                                                                                                                  |                                                       
| `--instance-sampling`            | Yes       | `None`                | The name of the strategy to be used for instance sub-sampling. Must be `with-replacement`, `without-replacement`, `stratified-label-wise`, `stratified-example-wise` or `None`. Additional options may be provided using the bracket notation `with-replacement{sample_size=0.5}`.                                                                                                                                    |
| `--recalculate-predictions`      | Yes       | `True`                | `True`, if the predictions of rules should be recalculated on the entire training data, if the argument `--instance-sampling` is not `None`, `False` otherwise.                                                                                                                                                                                                                                                       |
| `--feature-sub-sampling`         | Yes       | `without-replacement` | The name of the strategy to be used for feature sub-sampling. Must be `without-replacement` or `None`. Additional options may be provided using the bracket notation `without-replacement{sample_size=0.5}`.                                                                                                                                                                                                          |
| `--holdout`                      | Yes       | `None`                | The name of the strategy to be used for creating a holdout set. Must be `random`, `stratified-label-wise`, `stratified-example-wise` or `None`, if no holdout set should be used. Additional options may be provided using the bracket notation `random{holdout_set_size=0.5}`.                                                                                                                                       |
| `--feature-binning`              | Yes       | `None`                | The name of the strategy to be used for feature binning. Must be `equal-width`, `equal-frequency` or `None`, if no feature binning should be used. Additional options may be provided using the bracket notation `equal-width{bin_ratio=0.5,min_bins=2,max_bins=256}`.                                                                                                                                                |
| `--label-binning`                | Yes       | `auto`                | The name of the strategy to be used for gradient-based label binning. Must be `auto`, `equal-width` or `None`, if no label binning should be used. Additional options may be provided using the bracket notation `equal-width{bin_ratio=0.04,min_bins=1,max_bins=8}`. If `auto` is used, the most suitable strategy is chosen automatically based on the loss function and the type of rule heads.                    |
| `--pruning`                      | Yes       | `None`                | The name of the strategy to be used for pruning rules. Must be `irep` or `None`. Does only have an effect if the parameter `--instance-sampling` is not set to `None`.                                                                                                                                                                                                                                                |
...
| `--max-head-refinements`         | Yes       | `0`                   | The maximum number of times the head of a rule may be refined. Must be at least `1` or `0`, if the number of refinements should not be restricted.                                                                                                                                                                                                                                                                    |
| `--head-type`                    | Yes       | `auto`                | The type of the rule heads that should be used. Must be `single-label`, `complete` or `auto`. If `auto` is used, the most suitable type of rule heads is chosen automatically based on the loss function.                                                                                                                                                                                                             |
...
| `--predictor`                    | Yes       | `auto`                | The name of the strategy to be used for making predictions. Must `label-wise`, `example-wise` or `auto`. If `auto` is used, the most suitable strategy is chosen automatically based on the loss function.                                                                                                                                                                                                            |
...
| `--num-threads-rule-refinement`  | Yes       | `1`                   | The number of threads to be used to search for potential refinements of rules. Must be at least `1` or `0`, if the number of cores that are available on the machine should be used.                                                                                                                                                                                                                                  |
| `--num-threads-statistic-update` | Yes       | `1`                   | The number of threads to be used to calculate gradients and Hessians. Must be at least `1` or `0`, if the number of cores that are available on the machine should be used.                                                                                                                                                                                                                                           |
| `--num-threads-prediction`       | Yes       | `1`                   | The number of threads to be used to make predictions. Must be at least `1` or `0`, if the number of cores that are available on the machine should be used.                                                                                                                                                                                                                                                           |
