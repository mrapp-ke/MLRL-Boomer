...
| `--instance-sampling`        | Yes       | `bagging`                  | The name of the strategy to be used for instance sub-sampling. Must be `bagging`, `random-instance-selection`, `stratified-label-wise`, `stratified-example-wise` or `None`. Additional arguments may be provided as a dictionary, e.g. `bagging{'sample_size':0.5}`.                                                                                                                                                 |
...
```
venv/bin/python3 python/main_boomer.py --data-dir /path/to/data --output-dir /path/to/results/emotions --model-dir /path/to/models/emotions --dataset emotions --folds 10 --max-rules 1000 --instance-sampling bagging --feature-sub-sampling random-feature-selection --loss label-wise-logistic-loss --shrinkage 0.3 --pruning None --head-refinement single-label
```
