...
| `--label-sampling`           | Yes       | `None`                     | The name of the strategy to be used for label sampling. Must be `random-label-selection` or `None`. Additional arguments may be provided as a dictionary, e.g. `random-label-selection{'num_samples':5}`.                                                                                                                                                                                                             |                                                       
| `--instance-sampling`        | Yes       | `with-replacement`         | The name of the strategy to be used for instance sub-sampling. Must be `with-replacement`, `random-instance-selection`, `stratified-label-wise`, `stratified-example-wise` or `None`. Additional arguments may be provided as a dictionary, e.g. `with-replacement{'sample_size':0.5}`.                                                                                                                               |
...
```
venv/bin/python3 python/main_boomer.py --data-dir /path/to/data --output-dir /path/to/results/emotions --model-dir /path/to/models/emotions --dataset emotions --folds 10 --max-rules 1000 --instance-sampling with-replacement --feature-sub-sampling random-feature-selection --loss label-wise-logistic-loss --shrinkage 0.3 --pruning None --head-refinement single-label
```
