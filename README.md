...
| `--instance-sub-sampling`    | Yes       | `bagging`                  | The name of the strategy to be used for instance sub-sampling. Must be `bagging`, `random-instance-selection`, `stratified-label-wise`, `stratified-example-wise` or `None`. Additional arguments may be provided as a dictionary, e.g. `bagging{'sample_size':0.5}`.                                                                                                                                                 |
| `--recalculate-predictions`  | Yes       | `True`                     | `True`, if the predictions of rules should be recalculated on the entire training data, if the argument `--instance-sub-sampling` is not `None`, `False` otherwise.                                                                                                                                                                                                                                                   |
...
| `--label-binning`            | Yes       | `None`                     | The name of the strategy to be used for label binning. Must be `equal-width` or `None`, if no label binning should be used. Additional arguments may be provided as a dictionary, e.g. `equal-width{'bin_ratio':0.04,'min_bins':1,'max_bins':8`.                                                                                                                                                                      |
