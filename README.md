...
| `--early-stopping`           | Yes       | `None`                     | The name of the strategy to be used for early stopping. Must be `measure` or `None`, if no early stopping should be used. Additional arguments may be provided as a dictionary, e.g., `measure{'min_rules':100,'update_interval':1,'stop_interval':1,'buffer_size':25,'aggregation':'min','tolerance':0.001}`. Does only have an effect if the parameter `--holdout` is set to a value greater than `0`. |                                                                                
