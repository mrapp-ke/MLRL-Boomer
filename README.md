...
| `--label-binning`            | Yes       | `None`                     | The name of the strategy to be used for label binning. Must be `equal-width` or `None`, if no label binning should be used. Additional arguments may be provided as a dictionary, e.g. `equal-width{'bin_ratio':0.33,'min_bins':1,'max_bins':16`.                                 |
...
| `--num-threads`              | Yes       | `1`                        | The number of threads to be used for training. Must be at least `1` or `-1`, if the number of cores that are available on the machine should be used.                                                                                                                             |
| `--num-threads-prediction`   | Yes       | `1`                        | The number of threads to be used for making predictions. Must be at least `1` or `-1`, if the number of cores that are available on the machine should be used.                                                                                                                   |
