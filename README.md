...
| `--parallel-rule-refinement`  | Yes       | auto                | Whether potential refinements of rules should be search for in parallel or not. Must be `true`, `false` or `auto`, if the most suitable strategy should be chosen automatically based on the parameter --loss. For additional options refer to the documentation.                                                                 |
| `--parallel-statistic-update` | Yes       | auto                | Whether the gradients and Hessians for different examples should be calculated in parallel or not. Must be `true`, `false` or `auto`, if the most suitable strategy should be chosen automatically based on the parameter --loss. For additional options refer to the documentation.                                              |
| `--parallel-prediction`       | Yes       | true                | Whether the predictions for different examples should be obtained in parallel or not. Must be `true` or `false`. For additional options refer to the documentation.                                                                                                                                                               |
