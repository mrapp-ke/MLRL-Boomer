...
- `output_sampling` (Default value = `'none'`)
  ...
  - `'without-replacement'` The outputs to be considered when learning a new rule are chosen randomly. The following options may be provided using the {ref}`bracket notation<bracket-notation>`:
...
- `parallel_statistic_update` (Default value = `'false'`)
  ...
  - `'true'` Multi-threading is used to assess the correctness of predictions for different examples in parallel. The following options may be provided using the {ref}`bracket notation<bracket-notation>`:
