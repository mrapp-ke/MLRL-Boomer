...
namespace boosting {
    ...
    Float64StatisticsConfig::Float64StatisticsConfig(ReadableProperty<IHeadConfig> headConfig)
        : headConfig_(headConfig) {}

    std::unique_ptr<IClassificationStatisticsProviderFactory>
      Float64StatisticsConfig::createClassificationStatisticsProviderFactory(
    ...
    std::unique_ptr<IRegressionStatisticsProviderFactory>
      Float64StatisticsConfig::createRegressionStatisticsProviderFactory(
        ...
        return headConfig_.get().createRegressionStatisticsProviderFactory(featureMatrix, regressionMatrix,
                                                                           lossFactoryPtr, evaluationMeasureFactoryPtr);
    }
    ...
    std::unique_ptr<IRegressionStatisticsProviderFactory>
      Float64StatisticsConfig::createRegressionStatisticsProviderFactory(
        ...
        std::unique_ptr<IRegressionEvaluationMeasureFactory<float64>> evaluationMeasureFactoryPtr =
          lossConfig.createRegressionEvaluationMeasureFactory();
        return headConfig_.get().createRegressionStatisticsProviderFactory(
