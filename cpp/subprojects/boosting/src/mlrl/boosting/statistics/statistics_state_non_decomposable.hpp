...
namespace boosting {
    ...
    template<typename OutputMatrix, typename StatisticMatrix, typename QuantizationMatrix, typename ScoreMatrix,
             typename Loss>
    class NonDecomposableBoostingStatisticsState final
        : public AbstractBoostingStatisticsState<OutputMatrix, StatisticMatrix, QuantizationMatrix, ScoreMatrix, Loss> {
        protected:
        ...
        public:
             ...
             * @param quantizationPtr       An unique pointer to an object of template type `QuantizationMatrix` that
             *                              provides access to the gradients and Hessians
             ...
             */
            NonDecomposableBoostingStatisticsState(const OutputMatrix& outputMatrix,
                ...
                : AbstractBoostingStatisticsState<OutputMatrix, StatisticMatrix, QuantizationMatrix, ScoreMatrix, Loss>(
                    outputMatrix, std::move(statisticMatrixPtr), std::move(quantizationMatrixPtr),
                    std::move(scoreMatrixPtr), std::move(lossFunctionPtr)) {}
    };
