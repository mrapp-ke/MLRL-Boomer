...
namespace boosting {
    ...
    static inline std::unique_ptr<IDecomposableStatistics<IDecomposableRuleEvaluationFactory>> createStatistics(
      ...
      const IQuantizationFactory& quantizationFactory, const IDecomposableRuleEvaluationFactory& ruleEvaluationFactory,
    ...
    #pragma omp parallel for firstprivate(numExamples) firstprivate(lossRawPtr) firstprivate(outputMatrixPtr) \
      ...
      num_threads(multiThreadingSettings.numThreads)
        ...
        auto sparseDecomposable32BitVisitor =
          [&](const IQuantizationMatrix<SparseSetView<Statistic<float32>>>& quantizationMatrixPtr) {
            throw std::runtime_error("not implemented");
        };
        auto sparseDecomposable64BitVisitor =
          [&](const IQuantizationMatrix<SparseSetView<Statistic<float64>>>& quantizationMatrixPtr) {
            throw std::runtime_error("not implemented");
        };
        auto denseNonDecomposable32BitVisitor =
          [&](const IQuantizationMatrix<DenseNonDecomposableStatisticView<float32>>& quantizationMatrixPtr) {
            throw std::runtime_error("not implemented");
        };
        auto denseNonDecomposable64BitVisitor =
          [&](const IQuantizationMatrix<DenseNonDecomposableStatisticView<float64>>& quantizationMatrixPtr) {
            throw std::runtime_error("not implemented");
        };
    ...
    std::unique_ptr<IStatisticsProvider>
      DenseDecomposableClassificationStatisticsProviderFactory<StatisticType, VectorMath>::create(
        ...
        std::unique_ptr<IDecomposableStatistics<IDecomposableRuleEvaluationFactory>> statisticsPtr = createStatistics(
          std::move(lossPtr), std::move(evaluationMeasurePtr), *quantizationFactoryPtr_,
          *defaultRuleEvaluationFactoryPtr_, multiThreadingSettings_, labelMatrix, std::type_identity<VectorMath> {});
