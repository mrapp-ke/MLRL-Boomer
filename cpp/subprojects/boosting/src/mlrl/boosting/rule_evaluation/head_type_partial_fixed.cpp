...
namespace boosting {
    ...
    class Preset final : public IHeadConfig::IPreset<StatisticType> {
        ...
        public:
            ...
            Preset(ReadableProperty<ILabelBinningConfig> labelBinningConfig,
                   ReadableProperty<IMultiThreadingConfig> multiThreadingConfig, float32 outputRatio, uint32 minOutputs,
                   uint32 maxOutputs)
    ...
    std::unique_ptr<IHeadConfig::IPreset<float32>> FixedPartialHeadConfig::create32Bit() const {
        return std::make_unique<Preset<float32>>(labelBinningConfig_, multiThreadingConfig_, outputRatio_, minOutputs_,
                                                 maxOutputs_);
    }

    std::unique_ptr<IHeadConfig::IPreset<float64>> FixedPartialHeadConfig::create64Bit() const {
        return std::make_unique<Preset<float64>>(labelBinningConfig_, multiThreadingConfig_, outputRatio_, minOutputs_,
                                                 maxOutputs_);
    }
