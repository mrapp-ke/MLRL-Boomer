...
namespace boosting {
     ...
     */
    template<typename StatisticType, typename GradientIterator, typename HessianIterator>
    static inline void sortOutputWiseCriteria(typename SparseArrayVector<StatisticType>::iterator tmpIterator,
        ...
        for (uint32 i = 0; i < numOutputs; i++) {
            IndexedValue<StatisticType>& entry = tmpIterator[i];
            entry.index = i;
        ...
        std::partial_sort(tmpIterator, &tmpIterator[numPredictions], &tmpIterator[numOutputs],
                          CompareOutputWiseCriteria<IndexedValue<StatisticType>>());
    }
