...
namespace boosting {
    ...
    class Preset final : public IHeadConfig::IPreset<StatisticType> {
        ...
        public:
            ...
            Preset(ReadableProperty<ILabelBinningConfig> labelBinningConfig,
                   ReadableProperty<IMultiThreadingConfig> multiThreadingConfig, float32 threshold, float32 exponent)
    ...
    std::unique_ptr<IHeadConfig::IPreset<float32>> DynamicPartialHeadConfig::create32Bit() const {
        return std::make_unique<Preset<float32>>(labelBinningConfig_, multiThreadingConfig_, threshold_, exponent_);
    }

    std::unique_ptr<IHeadConfig::IPreset<float64>> DynamicPartialHeadConfig::create64Bit() const {
        return std::make_unique<Preset<float64>>(labelBinningConfig_, multiThreadingConfig_, threshold_, exponent_);
    }
