...
#include "mlrl/common/rule_refinement/rule_refinement_callback.hpp"
...
class ExactRuleRefinement final : public IRuleRefinement {
    ...
    public:
         ...
         * @param callbackPtr   An unique pointer to an object of type `IRuleRefinementCallback` that allows to retrieve
         *                      the information that is required to search for potential refinements
         */
        ExactRuleRefinement(const IndexVector& labelIndices, std::unique_ptr<Callback> callbackPtr);
