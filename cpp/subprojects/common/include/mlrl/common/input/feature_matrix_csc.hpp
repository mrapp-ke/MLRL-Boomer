 ...
 *                  feature values
 * @param indices   A pointer to an array of type `uint32`, shape `(numNonZeroValues)`, that stores the row-indices, the
 *                  values in `values` correspond to
 * @param indptr    A pointer to an array of type `uint32`, shape `(numCols + 1)`, that stores the indices of the first
 *                  element in `values` and `indices` that corresponds to a certain column. The index at the last
