...
/**
 * An abstract base class for all classes that allow to obtain predictions for given query examples.
 *
 ...
 */
template<typename PredictionMatrix, typename FeatureMatrix, typename Model>
class AbstractPredictor : virtual public IPredictor<PredictionMatrix> {
    private:
    ...
    protected:
         ...
         * @param predictionMatrix  A reference to an object of template type `PredictionMatrix` that should be used to
        ...
        virtual void predictForExample(const Model& model, const FeatureMatrix& featureMatrix,
                                       PredictionMatrix& predictionMatrix, uint32 maxRules,
