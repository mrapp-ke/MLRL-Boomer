...
std::unique_ptr<IMarginalProbabilityCalibrationModel> NoMarginalProbabilityCalibrator::fitProbabilityCalibrationModel(
  const SinglePartition& partition, const CContiguousLabelMatrix& labelMatrix, const IStatistics& statistics) const {
    return std::make_unique<NoMarginalProbabilityCalibrationModel>();
}

std::unique_ptr<IMarginalProbabilityCalibrationModel> NoMarginalProbabilityCalibrator::fitProbabilityCalibrationModel(
  const SinglePartition& partition, const CsrLabelMatrix& labelMatrix, const IStatistics& statistics) const {
    return std::make_unique<NoMarginalProbabilityCalibrationModel>();
}

std::unique_ptr<IMarginalProbabilityCalibrationModel> NoMarginalProbabilityCalibrator::fitProbabilityCalibrationModel(
  BiPartition& partition, const CContiguousLabelMatrix& labelMatrix, const IStatistics& statistics) const {
    return std::make_unique<NoMarginalProbabilityCalibrationModel>();
}

std::unique_ptr<IMarginalProbabilityCalibrationModel> NoMarginalProbabilityCalibrator::fitProbabilityCalibrationModel(
  BiPartition& partition, const CsrLabelMatrix& labelMatrix, const IStatistics& statistics) const {
    return std::make_unique<NoMarginalProbabilityCalibrationModel>();
}
