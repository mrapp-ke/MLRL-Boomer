...
void FullHead::apply(DensePredictionMatrix::iterator begin, DensePredictionMatrix::iterator end) const {
    for (uint32 i = 0; i < numScores_; i++) {
        begin[i] += scores_[i];
    }
}

void FullHead::apply(DensePredictionMatrix::iterator predictionsBegin, DensePredictionMatrix::iterator predictionsEnd,
                     Mask::iterator maskBegin, Mask::iterator maskEnd) const {
    for (uint32 i = 0; i < numScores_; i++) {
        if (maskBegin[i]) {
