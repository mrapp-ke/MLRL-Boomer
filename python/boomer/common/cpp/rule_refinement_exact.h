    ...
    public:
        ...
        void findRefinement(const AbstractEvaluatedPrediction* currentHead) override {
            ...
            if (sumOfWeights > 0) {
                for (r = r + 1; r < numElements; r++) {
                    ...
                    if (weight > 0) {
                        ...
                        if (previousThreshold != currentThreshold) {
                            ...
                            // Reset the subset in case of a nominal feature, as the previous examples will not be
                ...
                // feature value, or if not all examples have been iterated so far, we must evaluate additional
            ...
            // If the feature is nominal and the examples with feature values >= 0 that have been iterated so far do not
            // all have the same feature value, we must evaluate additional conditions `f == previous_threshold` and
            ...
            // If the sum of weights of all examples that have been iterated so far (including those with feature values
            // < 0 and those with feature values >= 0) is less than the sum of of weights of all examples, this means
            // that there are examples with sparse, i.e. zero, feature values. In such case, we must explicitly test
            // conditions that separate these examples from the ones that have already been iterated...
            if (totalAccumulatedSumOfWeights > 0 && totalAccumulatedSumOfWeights < totalSumOfWeights_) {
                // If the feature is nominal, we must reset the subset once again to ensure that the accumulated state
                // includes all examples that have been processed so far...
                if (nominal_) {
                    statisticsSubsetPtr->resetSubset();
                    firstR = ((intp) numElements) - 1;
                }
                ...
                // `f > previous_threshold / 2` (or the condition `f != 0` in case of a nominal feature) is used...
                const AbstractEvaluatedPrediction* head = headRefinementPtr_->findHead(bestHead, *statisticsSubsetPtr,
                                                                                       false, nominal_);
            ...
            // If the feature is numerical and there are other examples than those with feature values < 0 that have
            // been processed earlier, we must evaluate additional conditions that separate the examples with feature
            // values < 0 from the remaining ones (unlike in the nominal case, these conditions cannot be evaluated
            // earlier, because it remains unclear what the thresholds of the conditions should be until the examples
            // with feature values >= 0 have been processed).
            if (!nominal_ && accumulatedSumOfWeightsNegative > 0
                && accumulatedSumOfWeightsNegative < totalSumOfWeights_) {
                ...
                if (head != nullptr) {
                    ...
                    if (totalAccumulatedSumOfWeights < totalSumOfWeights_) {
                        ...
                        // If the condition separates an examples with feature value < 0 from an example with feature
                        // value > 0
                ...
                // Find and evaluate the best head for the current refinement, if the condition that uses the > operator
                // is used...
