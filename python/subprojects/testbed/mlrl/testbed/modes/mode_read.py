...
Provides classes that implement a mode of operation for performing reading experimental results.
"""
...
class ReadMode(Mode):
    ...
    def __check_version(meta_data: MetaData):
        ...
        log.debug(
            'Experimental results have been created with version "%s" of the package "mlrl-testbed", version '
            + '"%s" is currently used', meta_data_version, current_version)
    ...
    @staticmethod
    def __run_command(command: Command):
        log.info('Re-running experiment "%s"...\n', str(command))
    ...
    def run_experiment(self, args: Namespace, recipe: Recipe):
        ...
        if input_directory:
            ...
            self.__run_command(meta_data.command)
