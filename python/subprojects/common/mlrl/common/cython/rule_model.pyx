...
cdef class Complete32BitHead:
    """
    A head of a rule that predicts numerical scores, represented by 32-bit floating point values, for all available
    outputs.
    """

    def __cinit__(self, const float32[::1] scores not None):
        """
        :param scores: A contiguous array of type `float32`, shape `(num_predictions)` that stores the predicted scores
        """
...
cdef class Partial32BitHead:
    """
    A head of a rule that predicts numerical scores, represented by 32-bit floating point values, for a subset of the
    available outputs.
    """

    def __cinit__(self, const uint32[::1] indices not None, const float32[::1] scores not None):
        """
...
class RuleModelVisitor:
    ...
    @abstractmethod
    def visit_complete_32bit_head(self, head: Complete32BitHead):
        """
        Must be implemented by subclasses in order to visit the heads of rules that predict numerical scores,
        represented by 32-bit floating point values, for all available outputs.

        :param head: A `Complete32BitHead` to be visited
        """
        pass
    ...
    @abstractmethod
    def visit_partial_32bit_head(self, head: Partial32BitHead):
        """
        Must be implemented by subclasses in order to visit the heads of rules that predict numerical scores,
        represented by 32-bit floating point values, for a subset of the available outputs.

        :param head: A `Partial32BitHead` to be visited
        """
        pass
...
cdef class RuleList(RuleModel):
    ...
    cdef __visit_complete_32bit_head(self, const Complete32BitHeadImpl& head):
        ...
        cdef const float32[::1] values = <float32[:num_elements]>head.values_cbegin()
        self.visitor.visit_complete_32bit_head(Complete32BitHead.__new__(Complete32BitHead, values))
    ...
    cdef __visit_partial_32bit_head(self, const Partial32BitHeadImpl& head):
        ...
        cdef const float32[::1] values = <float32[:num_elements]>head.values_cbegin()
        self.visitor.visit_partial_32bit_head(Partial32BitHead.__new__(Partial32BitHead, indices, values))
