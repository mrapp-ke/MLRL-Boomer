...
class RuleLearnerIntegrationTestsMixin(IntegrationTests):
    ...
    def test_sparse_feature_value(self, dataset: Dataset):
        builder = self.create_cmd_builder(dataset=dataset.numerical_sparse).sparse_feature_value(1.0)
    ...
    def test_feature_format(self, dataset_name: str, feature_format: str, dataset: Dataset):
        builder = self.create_cmd_builder(dataset=getattr(dataset, dataset_name)).feature_format(feature_format)
    ...
    def test_output_format(self, output_format: str, dataset: Dataset):
        builder = self.create_cmd_builder(dataset=dataset.default).output_format(output_format)
    ...
    def test_instance_sampling(self, instance_sampling: str, dataset: Dataset):
        builder = self.create_cmd_builder(dataset=dataset.default).instance_sampling(instance_sampling)
    ...
    def test_feature_sampling(self, feature_sampling: str, dataset: Dataset):
        builder = self.create_cmd_builder(dataset=dataset.default).feature_sampling(feature_sampling)
    ...
    def test_output_sampling(self, output_sampling: str, dataset: Dataset):
        builder = self.create_cmd_builder(dataset=dataset.default).output_sampling(output_sampling)
        CmdRunner(builder).run(f'output-sampling-{output_sampling}')

    @pytest.mark.parametrize(
        'rule_pruning, instance_sampling',
        [
            (NONE, None),
    ...
    def test_rule_induction(self, rule_induction: str, dataset: Dataset):
        builder = self.create_cmd_builder(dataset=dataset.default).rule_induction(rule_induction)
    ...
    def test_post_optimization(self, post_optimization: str, dataset: Dataset):
        builder = self.create_cmd_builder(dataset=dataset.default).post_optimization(post_optimization)
    ...
    def test_feature_binning(self, feature_binning: str, dataset_name: str, feature_format: str, dataset: Dataset):
        builder = (
            self.create_cmd_builder(dataset=getattr(dataset, dataset_name))
