...
class TestbedIntegrationTestsMixin(IntegrationTests):
    ...
    def test_meka_format(self, dataset: Dataset):
        builder = self.create_cmd_builder(dataset=dataset.meka) \
            .print_evaluation(False)
        CmdRunner(builder).run('meka-format')

    def test_svm_format(self, dataset: Dataset):
        builder = self.create_cmd_builder(dataset=dataset.svm) \
            .print_evaluation(False)
    ...
    def test_evaluation(self, predefined: bool, dataset: Dataset):
        ...
        builder = self.create_cmd_builder(dataset=dataset.default + ('-predefined' if predefined else '')) \
            .save_meta_data() \
            .print_evaluation() \
            .save_evaluation()
        CmdRunner(builder).run(test_name, wipe_after=False)
