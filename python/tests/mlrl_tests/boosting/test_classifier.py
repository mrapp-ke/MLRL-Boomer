...
class TestBoomerClassifier(
    ClassificationIntegrationTests, ClassificationRuleLearnerIntegrationTestsMixin, BoomerIntegrationTestsMixin
):
    """
    ...
    def test_single_label(self, prediction_type: PredictionType, dataset: Dataset):
        builder = (
            self.create_cmd_builder(dataset=dataset.single_output).prediction_type(prediction_type).print_evaluation()
        )
    ...
    def test_loss(self, loss: str, statistic_type: str):
        builder = self.create_cmd_builder().loss(loss).statistic_type(statistic_type)
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            (
                BinaryPredictorParameter.BINARY_PREDICTOR_GFM,
                Options(),
                PROBABILITY_CALIBRATION_ISOTONIC,
                PROBABILITY_CALIBRATION_ISOTONIC,
                True,
    ...
    def test_predictor_binary(
        self,
        binary_predictor: str,
        binary_predictor_options: Options,
        marginal_probability_calibration: str | None,
        joint_probability_calibration: str | None,
        label_vectors: bool | None,
        prediction_format: str | None,
    ):
        test_name = (
            f'predictor-binary-{binary_predictor}'
        ...
        CmdRunner(builder).run(
            test_name, wipe_after=not marginal_probability_calibration and not joint_probability_calibration
        )
    ...
    def test_predictor_binary_incremental(
        self,
        binary_predictor: str,
        binary_predictor_options: Options,
        marginal_probability_calibration: str | None,
    ...
    def test_predictor_score_output_wise(self):
        builder = (
            self.create_cmd_builder().prediction_type(PredictionType.SCORES).print_predictions().print_ground_truth()
        )
    ...
    @pytest.mark.parametrize(
        ...
        [
            (
                ProbabilityPredictorParameter.PROBABILITY_PREDICTOR_OUTPUT_WISE,
                PROBABILITY_CALIBRATION_ISOTONIC,
                None,
                None,
            ),
            (
                ProbabilityPredictorParameter.PROBABILITY_PREDICTOR_MARGINALIZED,
                PROBABILITY_CALIBRATION_ISOTONIC,
                PROBABILITY_CALIBRATION_ISOTONIC,
                True,
            ),
    ...
    def test_predictor_probability(
        self,
        probability_predictor: str,
        marginal_probability_calibration: str | None,
        joint_probability_calibration: str | None,
        print_label_vectors: bool | None,
    ):
        builder = (
            self.create_cmd_builder()
            .marginal_probability_calibration(marginal_probability_calibration)
    ...
    @pytest.mark.parametrize(
        'probability_predictor, marginal_probability_calibration, joint_probability_calibration',
        [
    ...
    def test_predictor_probability_incremental(
        self,
        probability_predictor: str,
        marginal_probability_calibration: str | None,
        joint_probability_calibration: str | None,
    ...
    def test_statistics_sparse(self, output_format: str, dataset: Dataset):
        builder = (
            self.create_cmd_builder(dataset=dataset.numerical_sparse)
    ...
    def test_decomposable_head_type(self, head_type: str, label_binning: str | None, statistic_type: str):
        ...
        CmdRunner(builder).run(
            f'decomposable-{head_type}-heads'
    ...
    def test_non_decomposable_head_type(self, head_type: str, label_binning: str | None, statistic_type: str):
        ...
        CmdRunner(builder).run(
            f'non-decomposable-{head_type}-heads'
            + (f'_{label_binning}-label-binning' if label_binning else '')
            + f'_{statistic_type}-statistics'
        )
    ...
    def test_global_pruning_stratified_holdout(self, global_pruning: str, holdout: str):
        builder = (
            self.create_cmd_builder().global_pruning(global_pruning).holdout(holdout).print_model_characteristics()
        )
